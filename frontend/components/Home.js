...
function Home() {
  ...
  const movies = moviesList.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.slice(0, 250) + "..."} poster={IMG_URL_BASE + data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
